const CACHE_NAME = 'neon-chomp-v8';
...
/* Network first, cache second.
   ...
   This used to be cache first — `caches.match(req).then(resp => resp || fetch(req))` —
   which looks right for a game that should work on a train and is a trap. Once
   the shell was in the cache it was served forever, so a device that visited
   early could never see another version of the game again. There was no route
   by which a fix reached it.

   Going to the network first means a deploy lands on the next load; the cache
   is what makes it work with no signal, not what decides which version you get.
   The network gets a few seconds before the cached copy is used instead, so a
   weak signal doesn't leave someone staring at a blank screen. */
