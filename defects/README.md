...
It only works if it happens at the **track**. Cropping the 640 square afterwards
would be the same pixels enlarged — no new detail, a narrower field of view, and
a quiet lie in the diagnostics. So the pipeline is unchanged:

```
camera → MediaStreamTrack (zoomed here) → video → the analysed frame
       → squareFrame() → model
```

`zoom.mjs` greps `squareFrame` to prove it knows nothing about zoom, and greps
`look()` to prove the survey loop never touches camera constraints mid-drive.

### Asking and being obeyed are different things

`applyConstraints` is asked through `advanced`, because a plain constraint is a
requirement that some implementations reject outright. Then the result is **read
back from `getSettings`**, because a camera can accept a constraint and do
nothing with it — and that must not read as success:
...
find at 2× is a different observation from a find at 1× — same road, different
