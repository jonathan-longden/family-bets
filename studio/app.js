...
function finishRecording() {
  ...
  newTake(samples, sr, 'Take ' + (state.takes.length + 1));
}
...
async function newTake(samples, sampleRate, name) {
  const meta = {
...
function matchGains() {
  const cur = state.cur;
  if (!state.match || !cur || !cur.studio || !cur.report || cur.meta.lufs == null) return { raw: 1, studio: 1 };
  let raw = dbToLin(cur.meta.lufs - cur.report.lufs), studio = 1;
  let peak = 0;
  const ch = cur.raw.channels[0];
  for (let i = 0; i < ch.length; i += 7) { const a = Math.abs(ch[i]); if (a > peak) peak = a; }
  if (raw * peak > 0.99) { const s = 0.99 / (raw * peak); raw *= s; studio *= s; }
  return { raw, studio };
}
...
function exportName(cur) {
  ...
  return safe + (state.hearing === 'raw' ? ' (raw)' : ' (studio)') + '.wav';
}
...
async function updateStorageNote() {
  ...
  $('#storageNote').textContent = state.takes.length
    ? state.takes.length + ' take' + (state.takes.length === 1 ? '' : 's') + ', ' + mb(used) + quota + '.'
    : 'Nothing stored yet.';
}
